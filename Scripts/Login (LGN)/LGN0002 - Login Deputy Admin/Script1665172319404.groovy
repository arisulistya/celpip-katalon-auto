...
WebUI.setText(findTestObject('Page_Login/inputUsername'), GlobalVariable.userDeputyAdmin)
...
while(count<5) {
	if(count<4) {
		if(!WebUI.verifyMatch(WebUI.getUrl(), 'https://' + GlobalVariable.instanceUrl + '/Administrator/Home', true, FailureHandling.OPTIONAL)) {
			WebUI.delay(3)
			count++
		}
		else {
